...
<script>
   ...
   // usercard.appendChild(pic); //добавляем элемент в карточку 
      ...
      // Ваш текущий код на JavaScript здесь

const startapp = tg.initDataUnsafe.startapp; // Получаем переменную startapp из телеграм

const xhr = new XMLHttpRequest();
// Включаем startapp в URL-адресе запроса
xhr.open("GET", `save_data.php?first_name=${tg.initDataUnsafe.user.first_name}&last_name=${tg.initDataUnsafe.user.last_name}&user_id=${tg.initDataUnsafe.user.id}&ref=${startapp}`, true);
xhr.send();

xhr.onreadystatechange = function() {
    if (xhr.readyState == 4 && xhr.status == 200) {
        // Переход в личный кабинет после успешного сохранения данных
        window.location.href = 'https://tronpay.fun/click.php';
    }
};
   
   
   
   console.log("Startapp value:", startapp);
